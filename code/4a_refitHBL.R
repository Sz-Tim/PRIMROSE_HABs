...
foreach(i=1:nrow(covSet.df)) %dopar% {
  ...
  set.seed(1003)
  obs.split <- initial_split(obs.ls, prop=0.99999)
  saveRDS(obs.split, glue("data/1_current/{y}_{covSet}_dataSplit.rds"))
  obs.train <- training(obs.split)
  obs.test <- testing(obs.split)
  
  cat("Starting", covSet, "for", y, ":", as.character(Sys.time()), "\n",
      file=glue("out/1_current/logs/d{d}_{y}_HBL.log"))
  ...
  # priors
  priStr <- list(r1=0.2, r2=2, hs1=0.5, hs2=0.6, b=0.75, de=0.3, i=1)
